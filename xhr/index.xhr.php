	...
	*/
	...
	if(false !== $db_result){
		...
		if($db_result->num_rows > 0){
			...
			if(isset($_POST['pwd'])){
				$motDePasseMatch = preg_match("/[0-9A-Za-z]{8,20}/", $_POST['pwd']);
...
function insererUsager($db){
	...
	*/
	...
	if(1 === $motDePasseMatch){
		...
		$query = "INSERT INTO `usagers` (`pseudo`, `motdepasse`%s, `dateInscription`, `courriel`) VALUES ('{$_POST['usager']}', '{$_POST['pwd']}'%s, '$dateInscription', '');";

		if(isset($_POST['nomUsager'])){
			$nomUsagerMatch = preg_match("/[0-9A-Za-z]{1,40}/", $_POST['nomUsager']);
			if(1 === $nomUsagerMatch){
